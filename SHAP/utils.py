...
def text_predict(text,tokenizer,device,text_model):

    # Tokenize and encode the input
    input_ids = torch.tensor([tokenizer.encode(v, padding="max_length", max_length=300, truncation=True) for v in text]).to(device)
    attention_masks = (input_ids != 0).type(torch.int64).to(device)

    # Pass through the model
    outputs = text_model(input_ids, attention_masks).detach().cpu().numpy()
        ...
        for i, name in enumerate(shap_values.output_names):
            if name == "control":
                transparent_colors = {
                                        "white": [255, 255, 255],
                                        "red": [68, 206, 80],# green 
                                        "green": [68, 206, 80]# green                                
                                        }

                update_colors(
                    red_lch=[73.7, 80.2, 140.4], #green 
                    l_mid=40.0,
                    blue_lch=[100.0, 0.0, 0.0], #white
                    light_blue=[255.0, 255, 255], #white
                    light_red=[195, 253, 189], #green 
                ...
                transparent_colors = {
                                        "white": [255, 255, 255],
                                        "red": [255, 195, 0],  #yellow 
                                        "green": [255, 195, 0], #yellow 
                                    }

                update_colors(
                    red_lch=[82.0, 84.1, 84.3], #yellow 
                    l_mid=40.0,
                    blue_lch=[100.0, 0.0, 0.0], #white
                    light_blue=[255.0, 255, 255], #white
                    light_red=[255, 212, 122], #light yellow
                    transparent_colors=transparent_colors
                )
            else:
                ...
                transparent_colors = {
                                    "white": [255, 255, 255],
                                    "red": [187, 115, 233], #purple 
                                    "green": [187, 115, 233] #purple 
                                }

                update_colors(
                    red_lch=[60.5, 69.1, 315.1], #purple 
                    l_mid=40.0,
                    blue_lch=[100.0, 0.0, 0.0], #white
                    light_blue=[255.0, 255, 255], #white
                    light_red=[236, 199, 255], #ligh purple 
                    transparent_colors=transparent_colors
                )
